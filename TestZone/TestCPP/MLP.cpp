...
void destroyDoubleArray3D(double ***array, int *npl, int lenOfNPL) {
    for (int row = 0; row < lenOfNPL; row += 1) {
        for (int col = 0; col < npl[col - 1] + 1; col += 1) {
            delete[] array[row][col];//deletes an inner array of integer;
        }
        delete[] array[row];
    }
    delete[] array;
...
MLP *initiateMLP(int *npl, int lenOfD) {
    ...
    for (int i = 0; i < lenOfD; i += 1) {
        if (i == 0) {
            tabW[i] = new double *[0];
        } else {
            tabW[i] = new double *[npl[i - 1] + 1];
            for (int j = 0; j < npl[i - 1] + 1; j += 1) {
                tabW[i][j] = new double[npl[i] + 1];
                for (int k = 0; k < npl[i] + 1; k += 1) {
                    if (k == 0) {
                        tabW[i][j][k] = 0;
                    } else {
                        //dis(e)
...
double predictMLP(MLP *mlp, int *sample_inputs, int is_classification) {
    double total = 0.0;
    for (int i = 0; i < mlp->d[0]; i += 1) {
        mlp->X[0][i + 1] = sample_inputs[i];
    }
    for (int i = 1; i < mlp->L + 1; i += 1) {
        for (int j = 1; j < mlp->d[i] + 1; j += 1) {
            total = 0.0;
            for (int k = 0; k < mlp->d[i - 1] + 1; k += 1) {
    ...
    // mlp->X[mlp->L][1:] en python donc à faire gaffe! - Erwan
    // Faire une boucle for lors du call de la fonction ? - Juan
    return mlp->X[mlp->L][1];
}


void trainMLP(MLP *mlp, int **allSamplesInputs, int lenAllSamplesInputs, int lenOneSamplesInputs,
              int *allSamplesExpectedOutputs, int lenSamplesExpectedOutputs, float learningRate, int isClassification,
              int nbIter) {
    int k;
    ...
    for (int _ = 0; _ < nbIter; _++) {
        ...
        for (int L = mlp->L; L > 0; L -= 1) {
            for (int i = 1; i < mlp->d[L - 1] + 1; i += 1) {
                total = 0.0;
        ...
        for (int L = 1; L < mlp->L + 1; L += 1) {
            for (int i = 0; i < mlp->d[L - 1] + 1; i += 1) {
                for (int j = 0; j < mlp->d[L] + 1; j += 1) {
...
int main() {
    ...
    destroyIntArray2D(xMalloc, lenColX);
    delete[] yMalloc;
    destroy_mlp_model(mlp);
    return EXIT_SUCCESS;
}