...
class dif:
    ...
    def __init__(self, directory_A, directory_B=None, similarity="normal", px_size=50, sort_output=False,
                 show_output=False, delete=False, silent_del=False):
        """
    ...
    def _search_one_dir(directory_A, similarity="normal", px_size=50, sort_output=False, show_output=False,
                        delete=False):
        ...
        print(len(img_matrices_A))
        # find duplicates/similar images within one folder

        LENGTH = len(img_matrices_A)  # Number of iterations required to fill pbar
    ...
    def _search_two_dirs(directory_A, directory_B=None, similarity="normal", px_size=50, sort_output=False,
                         show_output=False, delete=False):
            ...
            for count_B, imageMatrix_B in enumerate(img_matrices_B):
                ...
                while rotations <= 3:
                    ...
                    if err < ref:
                        ...
                        if filenames_A[count_A] in result.keys():
                            result[filenames_A[count_A]]["duplicates"] = result[filenames_A[count_A]]["duplicates"] + [
                                directory_B + "/" + filenames_B[count_B]]
                        else:
                            result[filenames_A[count_A]] = {"location": directory_A + "/" + filenames_A[count_A],
                                                            "duplicates": [directory_B + "/" + filenames_B[count_B]]
                                                            }
                        high, low = dif._check_img_quality(directory_A, directory_B, filenames_A[count_A],
                                                           filenames_B[count_B])
                        lower_quality.append(low)
                        break
                    else:
    ...
    def _create_imgs_matrix(directory, px_size):
        ...
        imgs_matrix = []
        for filename in tqdm(folder_files):
