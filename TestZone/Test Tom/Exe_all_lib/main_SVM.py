...
if __name__ == '__main__':
    ...
    # Linear
    limit = [10000000]
    threshold = [0.5, 0.1, 0.01, 0.001, 0.0001, 0.00001]
    learning_rate = [0.49, 0.09, 0.009, 0.0009, 0.00009, 0.000009]
    for thr in threshold:
        for LA in learning_rate:
            ...
            res = training_SVM(my_dll, x_training, y_training, x_testing, y_testing, learning_rate, thr, limit)
            res_train = res[0]
            res_test = res[1]
            ...
            with open("D:/PA/models_save/mlp_txt/32_classique_max.txt", "a") as filout:
                filout.write(
                    f"{str(DS)};{str(thr)};{str(LA)};{str(res_train)};{str(res_test)};{str(diff_res)}")
